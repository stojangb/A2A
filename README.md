...
- **A2A Client:** An application or another agent that consumes A2A services. It sends requests (like `message/send`) to an A2A Server's URL.
- **Task:** The fundamental unit of work managed by A2A, identified by a unique ID generated by the server for messages sent by the client. Tasks are stateful and progress through a defined lifecycle. 
...
- **Streaming:** For long-running tasks, servers supporting the `streaming` capability can use `message/stream`. The client receives Server-Sent Events (SSE) containing `Message`, `Task`, `TaskStatusUpdateEvent` or `TaskArtifactUpdateEvent` messages, providing real-time progress.
- **Push Notifications:** Servers supporting `pushNotifications` can proactively send task updates to a client-provided webhook URL, configured via `tasks/pushNotificationConfig/set`.

### Typical Flow

1.  **Discovery:** Client fetches the Agent Card from the server's well-known URL.
2.  **Initiation:** Client sends a `message/send` or `message/stream` request containing the initial user message.
3.  **Processing:**
    - **(Streaming):** Server sends SSE events as a message or (status updates, artifacts) as the task progresses.
    - **(Non-Streaming):** Server processes the task synchronously and returns the final `Message` or `Task` object in the response.
4.  **Interaction (Optional):** If the task enters `input-required`, the client sends subsequent messages using the same Task ID via `message/send` or `message/stream`.
