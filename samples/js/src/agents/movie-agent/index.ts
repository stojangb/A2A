...
import { A2AExpressApp } from "../../server/a2a_express_app.js";
...
import { IExecutionEventBus } from "../../server/events/execution_event_bus.js";
import { DefaultRequestHandler } from "../../server/request_handler/default_request_handler.js";
...
class MovieAgentExecutor implements AgentExecutor {
  async execute(
    ...
    // Use history from existingTask if available, otherwise start with userMessage
    const historyForGenkit = existingTask?.history ? [...existingTask.history] : [];
    // Ensure the current userMessage is part of the history if not already (e.g., if existingTask didn't include it yet)
