...
export class A2AExpressApp {
    ...
    /**
     * Adds A2A routes to an existing Express app or creates a new one.
     * @param app Optional existing Express app.
     * @param baseUrl The base URL for A2A endpoints (e.g., "/a2a/api").
     * @returns The Express app with A2A routes.
     */
    public setupRoutes(app: Express = express(), baseUrl: string = ''): Express {
            ...
            try {
                ...
                if (typeof (rpcResponseOrStream as any)?.[Symbol.asyncIterator] === 'function') {
                    ...
                    if (rpcResponse.error) {
                        if (rpcResponse.error.code === -32700 || rpcResponse.error.code === -32600 || rpcResponse.error.code === -32601 || rpcResponse.error.code === -32602) {
                            res.status(400).json(rpcResponse);
                        } else if (rpcResponse.error.code === -32603) {
                            res.status(500).json(rpcResponse);
                        }
                        else {
