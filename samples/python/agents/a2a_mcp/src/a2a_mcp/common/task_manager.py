...
class AgentTaskManager(InMemoryTaskManager):
    def __init__(
        self, agent: BaseAgent, notification_sender_auth: PushNotificationSenderAuth
    ):
        super().__init__()
        config_logger(logger=logger)
        self.agent = agent
        self.notification_sender_auth = notification_sender_auth
        logger.info(f"Initializing AgentTaskManager for {self.agent.agent_name}")
    ...
    def _validate_request(
        ...
        if not utils.are_modalities_compatible(
            task_send_params.acceptedOutputModes, self.agent.content_types
        ):
            logger.warning(
                "Unsupported output mode. Received %s, Support %s",
                task_send_params.acceptedOutputModes,
                self.agent.content_types,
            )
    ...
    async def _process_agent_response(
        ...
        if agent_response["response_type"] == "text":
            parts = [
                {
                    "type": "text",
        ...
        elif agent_response["response_type"] == "data":
            parts = [
                {
                    "type": "data",
    ...
    async def _run_streaming_agent(self, request: SendTaskStreamingRequest):
        ...
        try:
            async for item in self.agent.stream(query, task_send_params.sessionId):
                ...
                artifact = None
                message = None
                if "response_type" in item and item["response_type"] == "data":
                    parts = [{"type": "data", "data": item["content"]}]
                else:
